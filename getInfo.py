...
#NOTE! SLIGHTLY ADAPTED AND UPDATED WORK OF "Dasister". Most of this is his. Original credit:
__author__ = 'Dasister'
__site__ = 'http://21games.ru'  #not up anymore )= , this is some cs:source old stuff
...
def GetInfo(addrPort):
	addr=addrPort[0]
	port=addrPort[1]
	query = SourceQuery(addr, port=port)
	res = query.get_info()
	if type(res)!=bool:
		return(res)
	else:
		return("ERROR-SERVER NOT REACHED")
		
		
if __name__ == '__main__':
	IP=input("IP: ")
	PORT=int(input("PORT: "))
	print(GetInfo(getIpPort.getIpPort(IP,PORT)))
